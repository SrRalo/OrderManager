drop policy if exists "categorias_select_auth" on public.categorias_menu;
create policy "categorias_select_auth"
on public.categorias_menu
for select
to authenticated
using (true);

drop policy if exists "categorias_insert_admin" on public.categorias_menu;
...
with check (
  exists (
    ...
    where u.id = auth.uid() and u.rol in ('admin', 'supervisor') and u.activo = true
  )
);

drop policy if exists "categorias_update_admin" on public.categorias_menu;
